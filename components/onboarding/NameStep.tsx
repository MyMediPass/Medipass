...
export default function NameStep({ data, updateFormData, onNext, onPrev }: NameStepProps) {
    const [name, setName] = useState(data.name || "");

    const handleNext = () => {
        if (name.trim() === "") {
            // Basic validation, can be enhanced with a toast or inline error message
    ...
    return (
        ...
        >
            <h2 className="text-3xl font-semibold text-center text-green-400">What should we call you?</h2>
            <Input
                id="name-input"
                type="text"
                value={name}
