...
export function MainNav() {
  ...
  const [isCollapsed, setIsCollapsed] = useState(() => {
    // Try to get the saved state from localStorage if available
  ...
  const allNavItems = isAdmin
    ? [...navItems, { name: "Admin Settings", href: "/admin/settings", icon: Settings }]
    : navItems
          ...
          <Sheet>
            ...
            <SheetContent side="left" className="w-64 p-0">
              ...
              <nav className="flex flex-col gap-1 p-4">
                ...
                <Button
                  variant="ghost"
                  className="flex items-center justify-start gap-3 rounded-md px-3 py-2 text-sm text-muted-foreground hover:bg-muted"
                  onClick={signOut}
                >
